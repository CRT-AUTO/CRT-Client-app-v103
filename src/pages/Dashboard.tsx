...
import { Facebook, Instagram, Bot, AlertTriangle, MessageCircle, Users, Clock, CheckSquare, BarChart2 } from 'lucide-react';
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Load data in parallel with proper error handling for each request
        const loadConnections = getSocialConnections().catch(err => {
          console.error('Error loading social connections:', err);
          return [];
        });
        
        const loadMappings = getVoiceflowMappings().catch(err => {
          console.error('Error loading Voiceflow mappings:', err);
          return [];
        });
        
        const loadStats = getDashboardStats(user.id).catch(err => {
        ...
        const loadAnalytics = getMessageAnalytics(user.id).catch(err => {
          console.error('Error loading message analytics:', err);
          return [];
        });
        
        const loadConversations = getRecentConversations(user.id).catch(err => {
          console.error('Error loading recent conversations:', err);
          return [];
        });
        
        const loadHourlyData = getMessageVolumeByHour(user.id).catch(err => {
